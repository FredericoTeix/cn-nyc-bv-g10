...
gcloud container clusters create ${CLUSTER_NAME} --zone ${REGION} --no-enable-basic-auth --preemptible \
  --cluster-version "1.21.11-gke.1100" --release-channel "stable" --machine-type "n2-standard-2" \
  ...
  --max-pods-per-node "110" --num-nodes "2" --enable-autoscaling --min-nodes "0" --max-nodes "6" --logging=SYSTEM,WORKLOAD --monitoring=SYSTEM --enable-ip-alias \
  --network "projects/${PROJECT}/global/networks/default" \
  --subnetwork "projects/${PROJECT}/regions/europe-west6/subnetworks/default" --no-enable-intra-node-visibility \
  ...
  --node-locations ${REGION} --enable-autoprovisioning --max-cpu 12 --max-memory 48
